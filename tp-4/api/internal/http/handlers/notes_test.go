...
func newTestRouter() http.Handler {
	mux, _ := newTestRouterWithEnricher()
	return mux
}

// newTestRouterWithEnricher is like newTestRouter but also returns the
// fakeEnricher so tests can assert Create/Update submitted a job.
func newTestRouterWithEnricher() (http.Handler, *fakeEnricher) {
	enricher := &fakeEnricher{}
	return middleware.RequestID(NewRouter(store.NewMemoryStore(), enricher)), enricher
}
...
func TestCreate_SubmitsEnrichmentJobAndReturnsPending(t *testing.T) {
	mux, enricher := newTestRouterWithEnricher()
	rec := doRequest(t, mux, http.MethodPost, "/api/v1/notes", core.CreateNoteInput{
		Title:   "Go",
		Content: "notes about go",
	})
	...
	if data["enrichment_status"] != "pending" {
		t.Fatalf("expected enrichment_status pending immediately after create, got %v", data["enrichment_status"])
	}

	jobs := enricher.submitted()
	if len(jobs) != 1 {
		t.Fatalf("expected 1 enrichment job submitted, got %d", len(jobs))
	}
	if jobs[0].NoteID != data["id"] || jobs[0].Title != "Go" {
		t.Fatalf("unexpected submitted job: %+v", jobs[0])
	}
}

func TestCreate_SucceedsEvenWhenEnrichmentQueueFull(t *testing.T) {
	mux := middleware.RequestID(NewRouter(store.NewMemoryStore(), rejectingEnricher{}))
	rec := doRequest(t, mux, http.MethodPost, "/api/v1/notes", core.CreateNoteInput{Title: "Go"})

	if rec.Code != http.StatusCreated {
		t.Fatalf("Create must still succeed when the enrichment queue is full, got %d", rec.Code)
	}
}
...
func TestUpdate_ResubmitsEnrichmentJob(t *testing.T) {
	mux, enricher := newTestRouterWithEnricher()
	created := decodeEnvelope(t, doRequest(t, mux, http.MethodPost, "/api/v1/notes", core.CreateNoteInput{Title: "Go"}))
	id := created.Data.(map[string]any)["id"].(string)
	...
	rec := doRequest(t, mux, http.MethodPatch, "/api/v1/notes/"+id, map[string]string{"title": "Go updated"})
	env := decodeEnvelope(t, rec)
	if env.Data.(map[string]any)["enrichment_status"] != "pending" {
		t.Fatalf("expected enrichment_status reset to pending after update, got %v", env.Data)
	}

	jobs := enricher.submitted()
	if len(jobs) != 2 {
		t.Fatalf("expected 2 enrichment jobs (create + update), got %d", len(jobs))
	}
	if jobs[1].Title != "Go updated" {
		t.Fatalf("expected the update job to carry the new title, got %+v", jobs[1])
	}
}
