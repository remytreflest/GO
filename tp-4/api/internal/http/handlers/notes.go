...
// Enricher submits a note for async enrichment without blocking the caller.
// It is satisfied by *enrichment.Pool; tests inject a fake to observe calls.
type Enricher interface {
	Submit(job enrichment.Job) bool
}
...
func NewNotesHandler(store core.Store, enricher Enricher) *NotesHandler {
	return &NotesHandler{Store: store, Enricher: enricher}
}
...
func (h *NotesHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	// Note write is synchronous and already committed; enrichment runs async
	// off this request. Submit never blocks: a full queue just logs and the
	// note stays "pending" until a later write retries enrichment.
	h.Enricher.Submit(enrichment.Job{NoteID: n.ID, Title: n.Title, Content: n.Content})
...
func (h *NotesHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	n, err := h.Store.Update(r.Context(), id, in)
	if err != nil {
		if errors.Is(err, core.ErrNotFound) {
			writeError(w, r, http.StatusNotFound, "not_found", "note not found", nil)
			return
	...
	// Any successful patch re-triggers enrichment (the store already reset
	// enrichment_status to pending) for the same async-write-back reason as Create.
	h.Enricher.Submit(enrichment.Job{NoteID: n.ID, Title: n.Title, Content: n.Content})
	writeJSON(w, r, http.StatusOK, n)
}
